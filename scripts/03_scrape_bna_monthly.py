...
def scrape_bna_quarterly(grammy_csv_path='data/processed/grammy_data_clean.csv',
    ...
    print("="*60)
    print("BEST NEW ARTIST BILLBOARD SCRAPER - MONTHLY SAMPLING")
    print("="*60)
    ...
    for i, artist in enumerate(remaining_artists[:artists_per_session], 1):
        ...
        # Generate monthly dates (12 samples per year)
        dates_to_check = []
        for year in range(start_year, end_year + 1):
            for month in ['01-01', '02-01', '03-01', '04-01', '05-01', '06-01',
                          '07-01', '08-01', '09-01', '10-01', '11-01', '12-01']:
                dates_to_check.append(f"{year}-{month}")
        
        print(f"  Checking {len(dates_to_check)} dates (monthly sampling)")
...
def combine_bna_files(output_dir='data/raw/billboard_bna'):
    ...
    # Save combined file
    output_path = 'data/processed/billboard_bna_monthly.csv'
...
if __name__ == "__main__":
    # Scrape Best New Artist winners with MONTHLY sampling
    # 66 total BNA winners, 10 artists per session
    # Estimated time per session: ~33-40 hours (1.5 days)
